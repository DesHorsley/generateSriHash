 ...
 * file, You can obtain one at https://mozilla.org/MPL/2.0/. */
 ...
 // This code was originally taken from: https://github.com/mozilla/srihash.org/blob/master/lib/helpers.js
 // Minor changes have been made to allow it to compile in this typescript configuration
  ...
  */
 const upgradeToHttps = (urlString: string): any => {
   ...
   if (urlObject.protocol === 'http:' &&
       secureHosts.includes(urlObject.hostname)) {
  ...
  */
 const moduleForProtocol = (urlString: string): any => {
  ...
  */
 const eligibility = (response:any ): Array<any> => {
   const issues = [];
  ...
  * @return {Function.cb}
  */
 const processResource = (data: string, response: any, resourceUrl: string, cb: Function): any => {
  ...
  */
 const fetchResource = (resourceUrl: string, cb: Function): any => {
   ...
   moduleForProtocol(resourceUrl).get(options, (response:any) => {
     let output;
  ...
  */
 const guessResourceType = (resource:any) => {
  ...
  */
 const getResourceTypeFromContentTypeHeader = (headerValue:any): string | false => {
  ...
  * @param {Function} cb - callback
  */
 const generateElement = (resourceUrl: string, algorithms: Array<any>, cb: Function) => {
   const options = {
     url: resourceUrl,
     algorithms
   };

   generate(options, (resource:any) => {
     let element;
     if (!resource.success) {
  ...
  */
 const shuffleArray = (oldArray:any) => {
